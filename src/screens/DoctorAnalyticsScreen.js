...
const CONDITION_KEYWORDS = [
  ...
  { key: 'bp', icons: ['amlodipine', 'enalapril', 'losartan', 'hydrochlorothiazide', 'nifedipine'], labelKey: 'condition_bp' },
  { key: 'hiv', icons: ['tenofovir', 'lamivudine', 'dolutegravir', 'tld', 'efavirenz', 'nevirapine'], labelKey: 'condition_hiv' },
];

const CONDITION_ICONS_MAP = { diabetes: 'water', bp: 'heart-pulse', hiv: 'shield-check', other: 'medical-bag' };
const TREND_ICONS = { improving: 'trending-up', worsening: 'trending-down', stable: 'trending-neutral', insufficient: 'help-circle-outline' };
...
export default function DoctorAnalyticsScreen() {
  ...
  async function loadPatientData(patientUid) {
    ...
    try {
      const [rx, adh, stk, meds, streak, tr] = await Promise.all([
        ...
        getPerMedicationAdherence(30, patientUid),
        getCurrentStreak(patientUid),
        getAdherenceTrend(30, patientUid),
      ]);
      setPrescriptions(rx);
      setAdherence(adh);
      setStreakDays(stk);
  ...
  const conditionAdherence = useMemo(() => {
    ...
    for (const [cond, rxs] of Object.entries(conditionGroups)) {
      ...
      result[cond] = { count: rxs.length, rate: total > 0 ? Math.round((taken / total) * 100) : 0, total, taken };
    }
    return result;
  }, [conditionGroups, perMed]);

  const trendColor = trend.direction === 'improving' ? COLORS.goal[500]
    : trend.direction === 'worsening' ? COLORS.error : COLORS.outline;
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top }]}>
      ...
      {patients.length === 0 ? (
        ...
        <>
          <TouchableOpacity style={styles.patientSelector} onPress={() => setShowPatientPicker(true)} activeOpacity={0.7}>
                ...
                {['diabetes', 'bp', 'hiv', 'other'].map(cond => {
                  ...
                  return (
                    <View key={cond} style={styles.condRow}>
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={styles.condName}>{t(condLabelMap[cond])}</Text>
                    ...
                    <View key={i} style={{ alignItems: 'center', gap: 4 }}>
                      <View style={[styles.streakDot, {
                        backgroundColor:
                          ...
                          day.status === 'missed' ? COLORS.error : COLORS.surfaceHigh,
                      }]} />
...
function getStyles(C) {
  return StyleSheet.create({
    ...
    patientPickerRow: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: RADIUS.md, marginBottom: 4 },
